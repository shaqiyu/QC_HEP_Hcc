...
def plotVars(config, signal_dataset, bkg_dataset, signal_weights, bkg_weights, outName):
    j = 0
    for variable in config["var_signal"]:
        ...
        hist_signal = getHistograms(
            config, signal_dataset, signal_weights, j, min, max, sample="signal"
        )
        hist_bkg = getHistograms(
            config, bkg_dataset, bkg_weights, j, min, max, sample="bkg"
        )
...
def plotROCcurve(tpr_QSVM, fpr_QSVM, aruc_QSVM, std_qauc, tpr_SVM, fpr_SVM, aruc_SVM, std_auc, tpr_IBM, fpr_IBM, aruc_IBM, name):
    ...
    graph_SVM.SetLineColor( ROOT.kOrange +7 )
    ...
    graph_IBM  = ROOT.TGraph( len(fpr_IBM), tpr_IBM,  1 - fpr_IBM )
    graph_IBM.SetLineColor( ROOT.kMagenta +4 )
    graph_IBM.SetLineWidth( 3 )
    graph_IBM.SetTitle( '' )
    graph_IBM.SetMarkerStyle( 21 )
    MultiGraph.Add(graph_IBM,"A")
    ...
    legend.AddEntry(graph_QSVM, "Quantum Simulator (AUC = %0.3f)" % (aruc_QSVM), "l")
    legend.AddEntry(graph_SVM, "Origin Hardware (AUC = %0.3f )" % (aruc_SVM), "l")
    legend.AddEntry(graph_IBM, "IBM Hardware (AUC = %0.3f )" % (aruc_IBM), "l")
