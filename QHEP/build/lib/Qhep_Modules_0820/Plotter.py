...
def plotVars(config, signal_dataset, bkg_dataset, outName):
    ...
    for variable in config["var_signal"]:
        ...
        canvas.Print("./plots/vvH/Variables/" + config["var_signal"][j] +"_"+ outName + ".pdf")
...
def plot_the_response(
    ...
    c.SaveAs("./plots/vvH/Responses/Response_" + method + ".pdf")
...
def plotOvertraining(
    ...
    c.SaveAs("./plots/vvH/Responses/OverTraining_" + method + ".pdf")
...
def plotROCcurve(tpr_QSVM, fpr_QSVM, aruc_QSVM, std_qauc, tpr_SVM, fpr_SVM, aruc_SVM, std_auc, name):
    ...
    legend.AddEntry(graph_QSVM, "QSVM-kernel (AUC = %0.3f \pm %0.3f)" % (aruc_QSVM, std_qauc), "l")
    legend.AddEntry(graph_SVM, "Classical SVM (AUC = %0.3f \pm %0.3f)" % (aruc_SVM, std_auc), "l")
    ...
    c2.Modified()
    c2.Update()
    c2.Print("./plots/vvH/ROCs/"+ name +".pdf")
...
def plotROC_IBM(tpr_QSVM, fpr_QSVM, aruc_QSVM, std_qauc, tpr_QSVM_ibm, fpr_QSVM_ibm, aruc_QSVM_ibm, std_qauc_ibm, tpr_SVM, fpr_SVM, aruc_SVM, std_auc, name):
    ...
    c2.Modified()
    c2.Update()
    c2.Print("./plots/vvH/ROCs/"+ name +"_comp.pdf")
